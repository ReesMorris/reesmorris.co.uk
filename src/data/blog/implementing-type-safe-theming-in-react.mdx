...
export interface StaticTokensInternal {
  theme?: string;
  colors: {
    transparent: ThemeValue; // we'll define this type shortly
    black: ThemeValue;
    white: ThemeValue;
  };
}
...
We didn't create a wrapper for the `setTheme` function in this case, but it can easily be updated to support multiple themes and be type-safe. Speaking of...
...
We _could_ create another prop in our ThemeProvider to accept this, but it's essentially another "global" CSS file - so let's update the ThemeProvider to accept an array of global styles instead.
